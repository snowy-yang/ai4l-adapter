# onebot-adapter

OneBot 11 正向 WebSocket 适配器, 并内置协议桥, 把 OneBot 翻译成极简协议对外暴露.

- 收: OneBot 实现 (NapCat / Lagrange / go-cqhttp 等) 通过正向 WebSocket 推送事件, 适配器接收并解析.
- 发: 调用 OneBot API (send_msg 等) 并等待响应, 通过 echo 字段配对请求与响应.
- 桥: `Server` 通过 WebSocket (msgpack) 把 OneBot 事件翻译推出, 把外部指令翻译转入.

## 安装

需要 Python >= 3.13.

```bash
uv sync
```

## 快速开始

### 作为 OneBot 客户端直接使用

```python
import asyncio
from onebot_adapter import Bot

async def main():
    bot = Bot("ws://127.0.0.1:3001", access_token=None)

    @bot.on_message()
    async def on_msg(event):
        if event.is_group:
            print(f"[群{event.group_id}] {event.user_id}: {event.message}")
        else:
            print(f"[私聊] {event.user_id}: {event.message}")
        await bot.send_msg(user_id=event.user_id, message="收到")

    await bot.run()

asyncio.run(main())
```

### 作为协议桥运行 (推荐)

从模板复制配置文件, 按需修改, 然后启动:

```bash
# 自动生成配置文件
python -m onebot_adapter --init
# 或指定路径生成
python -m onebot_adapter --init -c /path/to/config.toml
# 编辑 config.toml ...
uv run python -m onebot_adapter
# 或指定配置文件
uv run python -m onebot_adapter -c /path/to/config.toml
# 或直接用 python
python -m onebot_adapter -c config.toml
# 也可以用根目录 main.py
python main.py
```

也可以在代码中手动运行:

```python
import asyncio
import logging
from onebot_adapter import Bot, Server

logging.basicConfig(level=logging.INFO)

async def main():
    bot = Bot("ws://127.0.0.1:3001", access_token=None)
    server = Server(bot, host="127.0.0.1", port=8080)
    await server.run()

asyncio.run(main())
```

桥启动后, 你的代码用 websockets 连上来即可, 无需接触 OneBot 细节.

## 配置

配置文件为 TOML 格式, 模板在 `templates/config.toml`. 使用 tomlkit 解析, 保留注释与格式.

路径查找顺序:

1. 命令行 `--config` / `-c` 参数
2. 环境变量 `ONEBOT_ADAPTER_CONFIG`
3. 当前目录下的 `config.toml`

没有配置文件时使用全默认值.

```toml
# OneBot WebSocket 连接
[onebot]
ws_url = "ws://127.0.0.1:3001"
access_token = ""
reconnect_interval = 3.0

# 协议桥服务
[server]
host = "127.0.0.1"
port = 8080
ws_path = "/"

# 日志
[log]
level = "INFO"
```

| 段 | 字段 | 默认值 | 说明 |
|------|------|------|------|
| `[onebot]` | `ws_url` | `ws://127.0.0.1:3001` | OneBot 正向 WS 地址 |
| | `access_token` | `""` | 鉴权 token, 留空不发送 |
| | `reconnect_interval` | `3.0` | 断线重连间隔 (秒) |
| `[server]` | `host` | `127.0.0.1` | 监听地址 |
| | `port` | `8080` | 监听端口 |
| | `ws_path` | `/` | WebSocket 端点 |
| `[log]` | `level` | `INFO` | 日志级别 |

未知字段自动忽略.

## 架构

```
OneBot 实现                  onebot-adapter                        你的代码
                                  Bot
   │ WebSocket                    │
   ├──── 事件 ───────────► Connection ──► Dispatcher ──► Server._translate ──► WS / (msgpack) ──► websockets 客户端
   │                              │                                          │
   │                              ApiCaller ◄── _handle_action ◄─────────────┤
   ├◄── API 响应 ───────────────┤                                          │ (双向 msgpack 二进制帧)
   └──── API 调用 ──────────────►┘
```

### 模块

| 模块 | 职责 |
|------|------|
| `connection.py` | 正向 WebSocket 连接管理, 收发原始 JSON, 断线自动重连 |
| `api.py` | API 调用层, 通过 echo 字段配对请求与响应 |
| `event.py` | 事件解析与分发, Event / MessageEvent / NoticeEvent / RequestEvent |
| `message.py` | 消息段与消息列表, 支持字符串 / 段 / 字典互转 |
| `bot.py` | 组合以上三层, 提供便捷 API 封装与装饰器 |
| `server.py` | 协议桥, WebSocket (msgpack) 通道 |
| `config.py` | TOML 配置加载, 路径查找与默认值 |
| `cli.py` | 命令行入口, 解析参数 + 加载配置 + 启动服务 |

## 协议规范

两条通道共用同一套事件格式和指令格式, 只是传输编码不同:

连接 `WS /`, 升级为 WebSocket 后全走 msgpack 二进制帧. 事件和指令共用一条连接.

只有两种事件类型: `message` 和 `notice`. OneBot 的 request 事件并入 notice. 两者共用 `detail` / `sub` / `user_id` / `group_id` 四个核心字段, 其余字段按事件类型平铺.

#### message

```json
{
  "detail": "group",
  "sub": "normal",
  "user_id": 100,
  "group_id": 200,
  "message": [
    {"type": "text", "text": "hi "},
    {"type": "at", "qq": "1"}
  ],
  "message_id": 99,
  "self_id": 999
}
```

| 字段 | 类型 | 说明 |
|------|------|------|
| `detail` | str | `"group"` 或 `"private"` |
| `sub` | str | 子类型 (normal / anonymous / notice) |
| `user_id` | int | 发送者 QQ |
| `group_id` | int / null | 群号, 私聊为 null |
| `message` | array | 消息段数组 (见下) |
| `message_id` | int / null | 消息 ID, 可能缺失 |
| `self_id` | int | 机器人 QQ |

消息段统一对象格式, 不含 CQ 码:

| type | 字段 | 说明 |
|------|------|------|
| `text` | `text` | 纯文本 |
| `at` | `qq` | @某人 |
| `reply` | `id` | 回复消息 |
| `image` / `video` / `audio` / `voice` / `file` | `content` | 媒体内容, base64 字符串 |

媒体段统一用 `content` 字段承载 base64 编码的二进制数据, 不使用 URL 或文件路径. 事件侧适配器会自动下载 URL 或读取本地路径并转成 base64; 指令侧把 `content` 还原成 OneBot 的 `file: "base64://<content>"`.

#### notice

OneBot 的 notice 和 request 事件都输出为 `notice`. `detail` 对应 `notice_type` 或 `request_type`.

```json
{
  "detail": "poke",
  "sub": "abc",
  "user_id": 3,
  "group_id": 4
}
```

request 并入时多一个 `comment` 字段:

```json
{
  "detail": "friend",
  "sub": "add",
  "user_id": 5,
  "group_id": null,
  "comment": "我是谁"
}
```

| 字段 | 类型 | 说明 |
|------|------|------|
| `detail` | str | notice_type 或 request_type |
| `sub` | str | sub_type |
| `user_id` | int | 相关用户 QQ |
| `group_id` | int / null | 群号, 非群事件为 null |
| `comment` | str | 仅 request 并入时有, 验证信息 |

#### 其他

未知 `post_type` 原样透传: `{"type": "<post_type>", "data": <原始 raw>}`.

### 指令

客户端发 BINARY 帧, msgpack 编码:

```json
{
  "action": "send_msg",
  "params": {
    "group_id": 1,
    "message": [
      {"type": "text", "text": "hi"},
      {"type": "image", "content": "iVBORw0KGgo="}
    ]
  }
}
```

`action` 直接使用 OneBot API 名 (send_msg / send_private_msg / send_group_msg / get_login_info 等). `params` 透传给 OneBot.

`params.message` 使用与事件相同的消息段格式, 适配器自动翻译成 OneBot 嵌套格式. 媒体段的 `content` (base64) 会还原成 OneBot 的 `file: "base64://<content>"`.

响应 (服务端发 BINARY 帧):

```json
// 成功
{"ok": true, "data": {"message_id": 7}, "error": null}

// OneBot 返回错误
{"ok": false, "data": null, "error": {"retcode": 1000, "message": "参数错误"}}

// 请求本身无效
{"ok": false, "data": null, "error": {"retcode": -1, "message": "missing action"}}
```

TEXT 帧被忽略, 只处理 BINARY 帧.

## 客户端示例 (websockets + msgpack)

```python
import asyncio
import base64
import msgpack
import websockets

async def main():
    async with websockets.connect("ws://127.0.0.1:8080/") as ws:
        # 发消息
        req = {
            "action": "send_msg",
            "params": {"group_id": 123, "message": [{"type": "text", "text": "hello"}]},
        }
        await ws.send(msgpack.packb(req, use_bin_type=True))
        resp = msgpack.unpackb(await ws.recv(), raw=False)
        print(resp)  # {"ok": True, "data": {"message_id": 7}, "error": None}

        # 发图片
        with open("cat.png", "rb") as f:
            b64 = base64.b64encode(f.read()).decode()
        req = {
            "action": "send_msg",
            "params": {"group_id": 123, "message": [{"type": "image", "content": b64}]},
        }
        await ws.send(msgpack.packb(req, use_bin_type=True))
        resp = msgpack.unpackb(await ws.recv(), raw=False)
        print(resp)

        # 收事件 (与发指令在同一条连接上)
        while True:
            proto = msgpack.unpackb(await ws.recv(), raw=False)
            print(f"[{proto['type']}] {proto['data']}")

asyncio.run(main())
```

## API 参考

### Bot

```python
Bot(ws_url, *, access_token=None, reconnect_interval=3.0)
```

- `bot.on_message()` / `bot.on_notice()` / `bot.on_request()` — 装饰器, 注册事件处理器
- `await bot.send_msg(*, user_id=None, group_id=None, message)` — 发消息 (group_id 优先)
- `await bot.send_private_msg(user_id, message)`
- `await bot.send_group_msg(group_id, message)`
- `await bot.get_login_info()`
- `await bot.run()` — 阻塞运行
- `await bot.close()`

`message` 参数接受 `str` / `Message` / `list[MessageSegment]`.

### Server

```python
Server(bot, *, host="127.0.0.1", port=8080, ws_path="/")
```

- `await server.run()` — 启动 WebSocket 服务并阻塞运行 OneBot WS 连接
- `await server.close()`

### Event

`Event.from_raw(data)` 按 `post_type` 分流:

- `MessageEvent` — `.is_private` / `.is_group` / `.message` / `.user_id` / `.group_id`
- `NoticeEvent` — `.notice_type` / `.sub_type` / `.user_id` / `.group_id`
- `RequestEvent` — `.request_type` / `.sub_type` / `.user_id` / `.comment`

所有事件保留 `.raw` 原始字典.

### Message / MessageSegment

```python
MessageSegment.text("hi")
MessageSegment.at(12345)
MessageSegment.reply(99)
MessageSegment.image("file:///a.png", cache=0)

Message.from_raw(raw)  # str -> [text], list -> 段数组
str(message)           # 文本表示
```

## 配置

OneBot 实现侧需开启正向 WebSocket 服务, 例如 NapCat:

```json
{
  "network": {
    "websocket": {
      "enable": true,
      "host": "127.0.0.1",
      "port": 3001
    }
  }
}
```

如设置了 `access_token`, 构造 Bot 时传入同名参数.

## 开发

```bash
uv sync                       # 安装依赖
python -m pytest              # 运行测试 (155 个)
python -m ruff check .        # lint
python -m ruff format .       # 格式化
python -m pyright             # 类型检查
```

## 许可

MIT
